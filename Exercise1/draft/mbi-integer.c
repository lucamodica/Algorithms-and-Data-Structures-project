...
void bin_insertion_sort(Record **arr, int l, int r){
  ...
  for(i = l; i <= r; i++){
    ...
    pos = binary_search(arr, elem->integer_field, l, i);
...
void merge(Record **arr, int l, int m, int r){
  ...
  k = l;
  while(i < nL && j < nR){
    if(L[i]->integer_field <= R[j]->integer_field){
      *arr[k] = *L[i];
      i++;
    }
    else{
...
int main(int argc, char *argv[]) {
  ...
  print_array_int_first(recs,0,size-1);
  ...
  clock_gettime(CLOCK_REALTIME, &begin);
	...
	bin_insertion_sort(recs,0,size-1);
  ...
  printf("**Print ordered records with merge sort (sorted by INT FIELD)**\n\n");	
  print_array_int_first(recs,0,size-1);
